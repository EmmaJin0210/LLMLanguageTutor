...
class VocabDetector:
    ...
    def detect_vocab(self, tokens):
        vocab_used = []
        for token in tokens:
            for vocab in self.vocab_keys:
                if vocab not in vocab_used and self.calc_levenshtein_similarity(token, vocab) > 0.8:
                    vocab_used.append(vocab)
        return vocab_used
