...
from core.utils.language_utils import get_levels_below_inclusive, get_levels_above_exclusive
...
class DifficultyEstimationEngine(BaseEngine):
    def __init__(self, language, target_level, vocab_dict, grammar_dict, api_key=None, model="gpt-4", tokenizer=None, **hyperparams):
        if api_key is None:
            raise ValueError("An API key is required for the DifficultyEstimationEngine.")
    ...
    def _load_tokenizer(self):
        ...
        """
        try:
            # Attempt to load the tokenizer for the given model
            import tiktoken
